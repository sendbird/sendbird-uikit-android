 ...
 * @since 3.5.0
 */
public class NotificationChannelViewModel extends BaseViewModel implements OnPagedDataLoader<List<BaseMessage>>, LifecycleEventObserver {
    @NonNull
     ...
     * @since 3.5.0
     */
    public NotificationChannelViewModel(@NonNull String channelUrl, @Nullable MessageListParams messageListParams) {
        ...
        connect((user, e) -> {
            if (user != null) {
                GroupChannel.getChannel(channelUrl, (channel, e1) -> {
                    NotificationChannelViewModel.this.channel = channel;
                    if (e1 != null) {
    ...
    protected void onCleared() {
        super.onCleared();
        Logger.d("-- onCleared NotificationChannelViewModel");
    ...
    public List<BaseMessage> loadPrevious() throws Exception {
        ...
        Logger.i(">> NotificationChannelViewModel::loadPrevious()");
    ...
    private synchronized void notifyChannelDataChanged() {
        Logger.d(">> NotificationChannelViewModel::notifyChannelDataChanged()");
        channelUpdated.setValue(channel);
    }

    @UiThread
    private synchronized void notifyDataSetChanged(@NonNull String traceName) {
        Logger.d(">> NotificationChannelViewModel::notifyDataSetChanged()");
    ...
    private synchronized void initMessageCollection(final long startingPoint) {
        Logger.i(">> NotificationChannelViewModel::initMessageCollection()");
        ...
        this.collection = SendbirdChat.createMessageCollection(new MessageCollectionCreateParams(channel, this.messageListParams, startingPoint, new MessageCollectionHandler() {
            ...
            public void onMessagesAdded(@NonNull MessageContext context, @NonNull GroupChannel channel, @NonNull List<BaseMessage> messages) {
                Logger.d(">> NotificationChannelViewModel::onMessagesAdded() from=%s", context.getCollectionEventSource());
            ...
            public void onMessagesUpdated(@NonNull MessageContext context, @NonNull GroupChannel channel, @NonNull List<BaseMessage> messages) {
                Logger.d(">> NotificationChannelViewModel::onMessagesUpdated() from=%s", context.getCollectionEventSource());
                notifyDataSetChanged(context);
            }
            ...
            public void onMessagesDeleted(@NonNull MessageContext context, @NonNull GroupChannel channel, @NonNull List<BaseMessage> messages) {
                Logger.d(">> NotificationChannelViewModel::onMessagesDeleted() from=%s", context.getCollectionEventSource());
            ...
            public void onChannelDeleted(@NonNull GroupChannelContext context, @NonNull String channelUrl) {
                Logger.d(">> NotificationChannelViewModel::onChannelDeleted() from=%s", context.getCollectionEventSource());
            ...
            public void onHugeGapDetected() {
                Logger.d(">> NotificationChannelViewModel::onHugeGapDetected()");
            }

            @Override
            public void onChannelUpdated(@NonNull GroupChannelContext context, @NonNull GroupChannel channel) {
                Logger.d(">> NotificationChannelViewModel::onChannelUpdated() from=%s, url=%s", context.getCollectionEventSource(), channel.getUrl());
    ...
    private synchronized void disposeMessageCollection() {
        Logger.i(">> NotificationChannelViewModel::disposeMessageCollection()");
        if (this.collection != null) {
    ...
    public void markAsRead() {
        Logger.d(">> NotificationChannelViewModel::markAsRead()");
    ...
    public void onStateChanged(@NonNull LifecycleOwner source, @NonNull Lifecycle.Event event) {
        Logger.i(">> NotificationChannelViewModel::onStateChanged(%s)", event);
        switch (event) {
            case ON_RESUME:
                isVisible = true;
                markAsRead();
